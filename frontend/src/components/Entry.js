...
class Entry extends React.Component {
  ...
  redirectOnClick = () => {
    if (!this.props.isDetailsView) this.setState({ isRedirect: true });
  };
        ...
        {entry && (
          <Card className={classes.card}>
            <CardHeader
                ...
                <div className="containerSpread">
                  ...
                  <div>
                    <IconButton
                      ...
                      onClick={() => deleteEntity(entry.id, entityName)}
                      aria-label="Delete"
                    >
                      <DeleteIcon />
                    </IconButton>
              ...
              subheader={
                <span onClick={this.redirectOnClick} className="pointer">
                  {`Posted by ${entry.author} on ${new Date(
                    entry.timestamp
                  ).toLocaleString()}`}
                </span>
              }
...
const mapDispatchToProps = dispatch => ({
  deleteEntity: (entityId, entityName) =>
    dispatch(deleteEntity(entityId, entityName)),
