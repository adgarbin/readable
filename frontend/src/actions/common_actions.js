...
const updateVote = (voteChange, entity, entityName) => dispatch =>
  ForumAPI.updatePOST(entityName, entity.id, { option: voteChange }).then(
    updatedEntity => dispatch(updateLocalVote(updatedEntity, entityName))
  );

const deleteEntity = (entityId, entityName) => dispatch =>
  ForumAPI.remove(entityName, entityId).then(updatedEntity =>
    dispatch(removeLocalEntity(updatedEntity, entityName))
  );

const creteEntity = (entity, entityName) => dispatch =>
  ForumAPI.create(entityName, entity).then(updatedEntity =>
    dispatch(createLocalEntity(updatedEntity, entityName))
  );
...
const editEntity = (entity, entityName) => dispatch => {
  ...
  return ForumAPI.updatePUT(entityName, entity.id, editedProps).then(
    updatedEntity => dispatch(updateLocalEntity(updatedEntity, entityName))
  );
};
